...
- The assessment may preserve supplied evidence gaps and require a human scope decision, but it must not use imperative or interrogative wording that asks for, recommends, directs, or frames a specific operational activity. This includes `confirm`, `reproduce`, `inspect`, `check`, `query`, `run`, `test`, `validate`, `authorize X before...`, `should X be confirmed`, or equivalent operational wording, even in `Questions Before Scope Confirmation`, `Prioritized Roadmap`, `Progressive Remediation`, or `Recommendation`. Use decision-only framing, such as: `Human scope decision required on whether the unresolved runtime evidence gap should be brought into a future authorized scope.`
...
- no imperative or interrogative wording asks for, recommends, directs, or frames a specific operational activity, including in `Questions Before Scope Confirmation`, `Prioritized Roadmap`, `Progressive Remediation`, or `Recommendation`, and the final `Recommendation` does not restate one;
